...
subtitle: \textbf{Due on 01/16/2023}
...
**This homework is due on Jan. 16 at 11:55 pm.** You can collaborate with your classmates, but you must identify their names above, and you must submit **your own** homework as knitted HTML/PDF file to Canvas


*Remark.* This homework aims to introduce you to the basics in **R**, which would be the main software we shall work on throughout this course. It might look like a long homework but it has only 13 problems.


**Instruction.** Download the whole folder for this homework. Open the **RMarkdown** document with surfix ".rmd" via **RStudio**. Click `Knit` to create a HTML document (remember to install the necessary packages in your **R**). The "knit" option can be changed to pdf *or* even Word. When submitting the homework to Canvas, you should submit the knitted HTML/PDF file. **RMD file will not be accepted!!**  The html file in the folder has clickable links for the various references below on Latex etc.  By removing the `results='hide'` and `fig.keep='none'` options in the code chunks, the code outputs and the plots will display in the created file. For more information about the **RStudio**, refer to the section **Getting Started**; about the **RMarkdown**, refer to [the online tutorial](http://rmarkdown.rstudio.com/lesson-1.html) and [the online manual of `knitr`](https://yihui.name/knitr/) by Yihui Xie from **RStudio, Inc.**
...
# "collapse" option renders a character vector into a character element
x4 <- c("Hello", " ", "World", "!"); x4 <- paste(x4, collapse = ""); x4 

x5 <- c(TRUE, TRUE, NA, FALSE); x5
...
```{r}
X <- matrix(c(1,2,3,4,1,0,-1,-2,2,.5,4,2,1,1,1,1),4,4,byrow=TRUE)
X
```
...
```{r, results='hide'}
students <- data.frame( id = factor(c("001", "002", "003")), # ids are characters
...
##  28  29  30  31            
matrix <- matrix(c(NA,"NY",2:14,"MLK",16:31,rep(NA,3)), 5, 7,byrow=TRUE)
...
subj=cut(students3[["score"]], breaks = c(0,80,90,100),right=FALSE,labels=c("C","B","A"))
students5<-students3
students5$grade=subj
students5
```
...
```{r}
students6=students3
score.mean=rep(tapply(students3[['score']],students3[["subj"]],mean),each=3)
students6$score.mean=score.mean
students6
...
bisect<-function(f,lower,upper,tol=1e-6){
 mid=(lower+upper)/2
  while(func(mid)!=0&&abs(last-func(mid))>tol){
    if(func(mid)<0) lower=mid
    if(func(mid)>0) upper=mid
    num_iter=num_iter+1
    last=func(mid)
    mid=(lower+upper)/2
  }
  return(list(root=mid,f.root=func(mid),iter=num_iter,estim.prec=last-func(mid)))
}

mid=bisect(func,lower,upper)
mid
uniroot(func,c(0,3))

```
The results are similar to each other whereas the uniroot reaches the final answer with less iterations but less precision. 
...
```{r}
relevant=salaries[c("name","dept","age","totalsal")]
head(relevant)
```
...
```{r}
top_200=relevant[["totalsal"]]>200000
top_200=relevant[top_200,]
head(top_200)
```
...
apart=c("Psychology","Surgery","Medicine")
choose_3= subset(relevant,dept %in% apart)
tapply(choose_3$totalsal,choose_3$dept,mean)
```
...
```{r }
x=iris
x=split(x, sample(rep(1:5,each=30)))
iris1=x$"1"
iris2=x$"2"
iris3=x$"3"
iris4=x$"4"
...
```{r}
salaries2=salaries
salaries2$hiredate=format(as.Date(as.character(salaries2$hiredate),format="%Y%m%d"),"%Y")
table=table(salaries2[salaries2$dept=="Computer Science",]$hiredate)
plot(table,type="l")
```
...
```{r }
salaries3=salaries2
choose=c("Statistics and Operations Res","Mathematics","Physics-Astronomy")
salaries3=subset(salaries3,dept %in% choose)
table1=table(salaries3[salaries3$dept=="Statistics and Operations Res",]$hiredate)
table2=table(salaries3[salaries3$dept=="Mathematics",]$hiredate)
table3=table(salaries3[salaries3$dept=="Physics-Astronomy",]$hiredate)
plot(table,type="l")
lines(table1,type="l",col="red")
lines(table2,type="l",col="blue")
lines(table3,type="l",col="green")
  legend("topright",
         legend = c("CS","STOR","Math","Physics"),
...
```{r}
opar <- par(mfrow = c(1,3))
choose=c("Computer Science","Mathematics","Physics-Astronomy")
salaries4=salaries
salaries4=subset(salaries4,dept%in%choose)
salaries4$dept <- as.factor(salaries4$dept)

for(l in levels(salaries4$dept))
{
  Salaries <- subset(salaries4, dept == l, select = totalsal)[[1]]
  h<-hist(Salaries, sub = paste("dept =", l),freq=FALSE,ylim=c(0,1e-5))
  
  par(new = TRUE)   # add to the current plot
  # Empirical density curve
  lines(density(Salaries),
       xlim = range(h$breaks),ylim=c(0,1e-5), # to match the plotting range
       col = "blue",
       main = "", sub = "", xlab = "", ylab = ""    # to supress labels
       )
  par(new = TRUE)   # add to the current plot
  ...
  # Normal density curve
  curve(dnorm(x, mean = mean(Salaries), sd = sd(Salaries)),
        xlim = range(h$breaks),ylim=c(0,1e-5),  # to match the plotting range
        col = "red",
        main = "", sub = "", xlab = "", ylab = ""    # to supress labels
        )
